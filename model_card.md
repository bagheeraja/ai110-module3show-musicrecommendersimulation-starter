...
**Second weakness, discovered after adding the "reasons" explanation column (v1.0.2):** every single top-5 result, across all six stress-test profiles, listed `key` and `mode` as its top two matching features, with only a third feature varying. That's because `key` (an unnormalized 0-11 integer) and `mode` (a 0/1 binary) aren't scaled the same way as the 0-1 continuous features, so they dominate the raw Euclidean distance far more than intended — the actual "winners" of the ranking are, almost always, whichever tracks happen to share the seed's exact key and mode. In practice this means the `match_score` is quietly weighted much more heavily toward music-theory key/mode agreement than toward the qualities (energy, danceability, mood) a listener actually cares about when they think "similar song." Fixing this would mean normalizing every feature (or explicitly weighting them) before computing distance, not just when generating the human-readable reasons.

**v1.0.3 fix, and a third weakness it revealed:** I addressed the above by re-encoding `key` as `key_x = 0.5·cos(2π·key/12)` / `key_y = 0.5·sin(2π·key/12)` — a circular encoding (so key 11 and key 0 are correctly adjacent, not maximally distant) scaled to the same `[-0.5, 0.5]` range as every other feature. This measurably changed real rankings (two stress-test profiles swapped in different tracks in their lower slots), confirming the fix wasn't cosmetic. But `mode` still dominates the "reasons" almost everywhere — and now that it's properly scaled, that's a *different*, more fundamental issue: `mode` is binary, so roughly half the catalog ties the seed exactly on it (diff = 0), while continuous 0-1 features almost never hit an exact match. A binary feature will structurally out-compete continuous ones in a plain distance metric even at equal scale, simply because exact ties are far more likely. Properly fixing this would need either dropping binary/categorical features from the raw distance calculation (handling them as a separate match/no-match bonus) or explicitly down-weighting them — scale normalization alone isn't enough.
